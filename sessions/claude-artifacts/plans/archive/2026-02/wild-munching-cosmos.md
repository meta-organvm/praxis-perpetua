...
The local directory `~/world/realm/operate/org/liminal/repo/a-i-skills` is **not a git repository**. It has no `.git` directory and no remote configured. The git commands that appear to work are actually resolving to a home-level repo at `~/.git`, which is unrelated.
...
```bash
rm -rf ~/world/realm/operate/org/liminal/repo/a-i-skills
```
...
```bash
gh repo clone organvm-iv-taxis/a-i--skills ~/world/realm/operate/org/liminal/repo/a-i-skills
```
...
## Files affected
- `~/world/realm/operate/org/liminal/repo/a-i-skills/` — deleted and replaced

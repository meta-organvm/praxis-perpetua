...
date: 2026-03-07
topic: "Systemic Project-Product Analysis Process — Two-Lane Diagnostic (Build Truth + World Interface)"
tags:
  - project-analysis
  - product-audit
  - two-lane-scorecard
  ...
  - diagnostic-process
content_hash: cfabb6e573f467d8ed40d37dfa8090cf6f6eeae7d8875a0b301dfd35beb13873
ingested_via: claude-code-manual
original_file: organ-logic-Systemic-Project-Analysis.md
status: reference
cross_references:
...
# Systemic Project-Product Analysis Process — Two-Lane Diagnostic

> Ingested from ChatGPT Q/A transcript. Defines a reusable analysis process for evaluating any project/product across two parallel lanes: Lane A (The Thing Itself — build quality, architecture, UX, defensibility) and Lane B (The Thing In The World — positioning, discovery, monetization, community fit). Includes 6-phase cycle, 14-dimension scorecard, diagnostic methods, mechanism ledger, monetization integrity tests, and portfolio-level operations.

## Relationship to ORGANVM North Star

This framework is the **diagnostic instrument** for the vision declared in [`meta-organvm/VISION.md`](../VISION.md). Where the [Creative Leadership Framework](./2026-03-07-creative-leadership-framework.md) defines how to *build* things and the [Era of the Orchestrator](./2026-03-07-era-of-the-orchestrator.md) argues *why* this model works, this document defines how to *evaluate* whether what you built actually delivers on its promise — both internally (Lane A: does the thing work?) and externally (Lane B: can the world find, trust, and pay for it?).

The North Star pillars surface here as:

| North Star Pillar | Two-Lane Diagnostic Mechanism |
|---|---|
| Anti-Stagnation Governance | The mismatch index rule: if `abs(mean_A - mean_B) ≥ 1.0`, stop adding features and fix the weaker lane. Prevents build-in-a-vacuum stagnation. |
| Ethical Human-in-Loop | Claim stack integrity: every claim needs proof or gets downgraded to hypothesis. No marketing vapor. The monetization integrity tests enforce honest value-to-cash paths. |
| Individual→Enterprise Amplification | Portfolio-level operation (§8): standardized scorecards across all Things let one practitioner manage 105 repos rationally via `$PORTFOLIO_NORTH_STAR`, `$CURRENT_FOCUS_SET`, and the leverage rule. |

The $-prefixed artifact IDs (`$SCORECARD`, `$CLAIM_STACK`, `$FRICTION_MAP`, `$VALUE_TO_CASH_MAP`) complement the Creative Leadership Framework's artifacts and are candidates for Conductor workflow steps.
